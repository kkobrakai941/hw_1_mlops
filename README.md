...
Проект инференса обученной модели CatBoost — предсказываем вероятность фрода по транзакциям. 
...
1. **app/app.py** — Запускает сначала препроцессинг, потом скоринг.  
...
7. **output/** — папка, куда сохраняются все результаты.  
